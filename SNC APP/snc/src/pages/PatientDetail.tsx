    ...
    <div className="space-y-5">
        ...
        <div className="space-y-4">
          <div className="flex justify-end">
            <button onClick={() => setShowModal(true)}
              className="flex items-center gap-2 px-4 py-2 bg-[#1a7a4a] text-white text-xs font-semibold rounded-lg hover:bg-[#0d4a2c]">
              + New Session
            </button>
          </div>
          {sessionsLoading ? (
      ...
      {showRegularPlanModal && (
        <RegularPlanModal
          ...
          onSaved={() => { setShowRegularPlanModal(false); loadRegularPlan(); emitAppEvent("app:regular-visits-changed"); emitAppEvent("app:patients-changed"); }}
        />
      )}
    </div>
  );
