...
const DB_PATH = path.join(DATA_DIR, 'snc.db');
let db;
...
function createTables() {
  ...
  d.exec(`
    ...
    CREATE TABLE IF NOT EXISTS sessions (
      ...
      date TEXT NOT NULL,
      visit_type TEXT NOT NULL DEFAULT 'IN-CLINIC',
      clinician_id TEXT,
      clinician_name TEXT,
      duration INTEGER,
    ...
    CREATE TABLE IF NOT EXISTS regular_visits (
      ...
      payment REAL,
      payment_mode TEXT,
      condition_status TEXT DEFAULT 'NOT_ASSESSED',
      updated_at TEXT,
      created_at TEXT NOT NULL
    );
    ...
    CREATE TABLE IF NOT EXISTS telegram_config (
      ...
      bot_token_enc TEXT,
      chat_id TEXT,
      alert_backup INTEGER NOT NULL DEFAULT 1,
      alert_login_fail INTEGER NOT NULL DEFAULT 1,
      alert_time TEXT NOT NULL DEFAULT '20:00',
    ...
    CREATE TABLE IF NOT EXISTS telegram_commands (
      ...
      response_type TEXT NOT NULL DEFAULT 'STATIC_TEXT',
      response_text TEXT,
      data_query TEXT,
  ...
  const insertPerm = d.prepare('INSERT OR IGNORE INTO permissions (id, role, screen, level) VALUES (?, ?, ?, ?)');
  const { v4: uid } = require('uuid');
  for (const [role, screen, level] of defaultPerms) insertPerm.run(uid(), role, screen, level);

  // Seed default admin
  const adminExists = d.prepare('SELECT id FROM users WHERE login_id = ?').get('admin');
  if (!adminExists) {
    const bcrypt = require('bcryptjs');
    d.prepare(`INSERT INTO users (id, login_id, password_hash, name, role, active, must_change_password, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
      uid(), 'admin', bcrypt.hashSync('admin123', 10), 'System Admin', 'ADMIN', 1, 0,
      new Date().toISOString(), new Date().toISOString()
    );
  }

  // Seed app_config defaults
  const insertCfg = d.prepare('INSERT OR IGNORE INTO app_config (id, key, value, created_at, updated_at) VALUES (?, ?, ?, ?, ?)');
  insertCfg.run('cfg_login_bypass', 'login_bypass', 'false', new Date().toISOString(), new Date().toISOString());
}

function now() { return new Date().toISOString(); }

module.exports = { getDb, createTables, now };
